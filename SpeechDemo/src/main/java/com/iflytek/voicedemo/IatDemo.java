...
public class IatDemo extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSpeechRecognizer = SpeechRecognizer.createRecognizer(IatDemo.this, mInitListener);
	...
	public void onClick(View view) {
		...
		case R.id.iat_recognize:
			...
			if (isShowDialog) {
				...
				// 不显示听写对话框
				mStartRet = mSpeechRecognizer.startListening(mRecognizerListener);
				if (mStartRet != ErrorCode.SUCCESS) {
					showTip("听写失败,错误码：" + mStartRet +",请点击网址https://www.xfyun.cn/document/error-code查询解决方案");
				} else {
					showTip(getString(R.string.text_begin));
				}
			}
...
//			mgr.asyncQueryAllContactsName();
		...
		case R.id.iat_upload_userwords:
			...
			mStartRet = mSpeechRecognizer.updateLexicon("userword", contents, mLexiconListener);
			if (mStartRet != ErrorCode.SUCCESS)
				showTip("上传热词失败,错误码：" + mStartRet +",请点击网址https://www.xfyun.cn/document/error-code查询解决方案");
			break;
		default:
			break;
		}
	...
	private RecognizerListener mRecognizerListener = new RecognizerListener() {
		...
		public void onResult(RecognizerResult results, boolean isLast) {
			...
			if (resultType.equals("json")) {
				...
				mStringBuffer.append(results.getResultString());
				mResultText.setText(mStringBuffer.toString());
...
//			Log.d(TAG, "返回音频数据："+data.length);
	...
	private ContactListener mContactListener = new ContactListener() {
		...
		public void onContactQueryFinish(final String contactInfos, boolean changeFlag) {
			...
			mSpeechRecognizer.setParameter(SpeechConstant.ENGINE_TYPE,SpeechConstant.TYPE_CLOUD);
			mSpeechRecognizer.setParameter(SpeechConstant.TEXT_ENCODING, "utf-8");
			mStartRet = mSpeechRecognizer.updateLexicon("contact", contactInfos, mLexiconListener);
			if (mStartRet != ErrorCode.SUCCESS) {
				showTip("上传联系人失败：" + mStartRet);
			}
		}
	};
	...
	public void setParam() {
		...
		// 设置返回结果格式
		mSpeechRecognizer.setParameter(SpeechConstant.RESULT_TYPE, resultType);
	...
	private void executeStream() {
		...
		// mSpeechRecognizer.setParameter(SpeechConstant.AUDIO_SOURCE, "-2");
		 //mSpeechRecognizer.setParameter(SpeechConstant.ASR_SOURCE_PATH, "sdcard/XXX/XXX.pcm");
		mStartRet = mSpeechRecognizer.startListening(mRecognizerListener);
		if (mStartRet != ErrorCode.SUCCESS) {
			showTip("识别失败,错误码：" + mStartRet +",请点击网址https://www.xfyun.cn/document/error-code查询解决方案");
		} else {
			...
			if (null != audioData) {
				...
				// 音频切分方法：FucUtil.splitBuffer(byte[] mStringBuffer,int length,int spsize);
